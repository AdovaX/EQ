/*! Summernote v0.8.8 | (c) 2013- Alan Hong and other contributors | MIT license */ ! function(a) {
	...
	if (!i && c)
		...
		} catch (a) {} else if ("undefined" != typeof require)
			if (void 0 !== require.resolve) try {
				require.resolve("codemirror"), i = !0
			} catch (a) {} else void 0 !== require.specified && (i = require.specified("codemirror"));
			...
			var b = function(b) {
				...
				ma = function(a, b, c) {
					...
					return d.length ? 1 === d.length ? la(b, c) : d.reduce(function(a, d) {
						return a === b.node && (a = la(b, c)), la({
							node: d,
							offset: a ? o.position(a) : G(d)
						}, c)
					}) : null
				},
				na = function(a, b) {
	...
	var q = function(b, c, d, e) {
		...
		w = r.create('<div class="note-editable card-block" contentEditable="true"/>'),
		...
		F = r.create('<div class="modal" aria-hidden="false" tabindex="-1"/>', function(a, b) {
			b.fade && a.addClass("fade"), a.html(['<div class="modal-dialog">', '  <div class="modal-content">', b.title ? '    <div class="modal-header">      <h4 class="modal-title">' + b.title + '</h4>      <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>    </div>' : "", '    <div class="modal-body">' + b.body + "</div>", b.footer ? '    <div class="modal-footer">' + b.footer + "</div>" : "", "  </div>", "</div>"].join(""))
		}),
		...
		H = r.create('<label class="custom-control custom-checkbox"></label>', function(a, b) {
			b.id && a.attr("for", b.id), a.html([' <input type="checkbox" class="custom-control-input"' + (b.id ? ' id="' + b.id + '"' : ""), (b.checked ? " checked" : "") + "/>", ' <span class="custom-control-indicator"></span>', ' <span class="custom-control-description">' + (b.text ? b.text : "") + "</span>", "</label>"].join(""))
		}),
		I = function(a, b) {
		...
		J = {
			...
			options: {},
			button: function(a, b) {
				return r.create('<button type="button" class="note-btn btn btn-primary btn-sm" tabindex="-1">', function(a, b) {
					b && b.tooltip && self.options.tooltip && a.attr({
						title: b.tooltip
					}).tooltip({
						container: "body",
						trigger: "hover",
			...
			var b = function(a, b) {
				...
				d = function(b, e, f, g) {
					...
					this.isOnEditable = i(o.isEditable), this.isOnList = i(o.isList), this.isOnAnchor = i(o.isAnchor), this.isOnCell = i(o.isCell), this.isOnData = i(o.isData), this.isLeftEdgeOf = function(a) {
						...
						return {
							s: {
								path: o.makeOffsetPath(a, b),
								offset: e
							},
							e: {
								path: o.makeOffsetPath(a, f),
								offset: g
							}
						}
						...
						return {
							s: {
								path: m.tail(o.makeOffsetPath(m.head(a), b)),
								offset: e
							},
							e: {
								path: m.tail(o.makeOffsetPath(m.last(a), f)),
								offset: g
							}
						}
	...
	var S, T = function() {
			this.tab = function(a, b) {
				...
				for (var c = o.ancestor(b.commonAncestor(), o.isCell), d = a(c).closest("tr"), e = d.children("td, th").index(a(c)), f = d[0].rowIndex, g = new R(c, R.where.Row, R.requestAction.Delete, a(d).closest("table")[0]), h = g.getActionList(), i = 0; i < h.length; i++)
					if (h[i]) {
						var j = h[i].baseCell,
							k = h[i].virtualTable,
				...
				for (var c = o.ancestor(b.commonAncestor(), o.isCell), d = a(c).closest("tr"), e = d.children("td, th").index(a(c)), f = new R(c, R.where.Column, R.requestAction.Delete, a(d).closest("table")[0]), g = f.getActionList(), h = 0; h < g.length; h++)
					if (g[h]) switch (g[h].action) {
						case R.resultAction.Ignore:
							continue;
						case R.resultAction.SubtractSpanCount:
							var i = g[h].baseCell,
		...
		U = function(b) {
			...
			this.fontSize = function(b) {
				...
				if (c && c.isCollapsed()) {
					...
					a(d).css({
						"font-size": b + "px"
					}), e && !o.nodeLength(e) && (e.innerHTML = o.ZERO_WIDTH_NBSP_CHAR, L.createFromNodeAfter(e.firstChild).select(), f.data("bogus", e))
				} else t(), a(n.styleNodes(c)).css({
					"font-size": b + "px"
				}), u()
				...
				if (a) {
					var b = m.find(m.from(a.childNodes), o.isText),
						c = b.nodeValue.indexOf(o.ZERO_WIDTH_NBSP_CHAR); - 1 !== c && b.deleteData(c, 1), o.isEmpty(a) && o.remove(a), f.removeData("bogus")
				}
			}, this.lineHeight = this.wrapCommand(function(a) {
				n.stylePara(this.createRange(), {
					lineHeight: a
				})
	...
	var X = function(a) {
		...
		Z = function(b) {
			...
			this.resizeTo = function(a) {
				...
				c.resizeTo({
					h: h.height() - e.outerHeight()
				})
			}, this.toggle = function() {
				d.toggleClass("fullscreen"), this.isFullscreen() ? (f.data("orgHeight", f.css("height")), h.on("resize", this.onResize).trigger("resize"), i.css("overflow", "hidden")) : (h.off("resize", this.onResize), this.resizeTo({
					h: f.data("orgHeight")
				}), i.css("overflow", "visible")), b.invoke("toolbar.updateFullscreen", this.isFullscreen())
			...
			this.initialize = function() {
				...
				this.isFontInstalled = function(a) {
					...
					for (var e = 0, f = c.length; e < f; e++) {
						for (var g = c[e], h = g[0], i = g[1], j = d.buttonGroup({
								className: "note-" + h
					...
					h.css({
						width: k.c + "em",
						height: k.r + "em"
					}), g.data("value", k.c + "x" + k.r), 3 < k.c && k.c < f.insertTableMaxSize.col && i.css({
						width: k.c + 1 + "em"
					}), 3 < k.r && k.r < f.insertTableMaxSize.row && i.css({
						height: k.r + 1 + "em"
					}), e.html(k.c + " x " + k.r)
				}
		},
		...
		ja = function(b) {
			...
			this.initialize = function() {
				...
				} else if (g && g[0].length) c = a("<iframe>").attr("frameborder", 0).attr("src", "https://instagram.com/p/" + g[1] + "/embed/").attr("width", "612").attr("height", "710").attr("scrolling", "no").attr("allowtransparency", "true");
